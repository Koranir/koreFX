...
texture NormTex  { Width = BUFFER_WIDTH / DOWNSAMPLE_BY ; Height = BUFFER_HEIGHT / DOWNSAMPLE_BY; Format = RGBA16f; };
sampler sNormTex { Texture = NormTex; };

texture NormTex1  { Width = BUFFER_WIDTH / DOWNSAMPLE_BY ; Height = BUFFER_HEIGHT / DOWNSAMPLE_BY; Format = RGBA16f; };
sampler sNormTex1 { Texture = NormTex1; };

texture VarianceTex { Width = BUFFER_WIDTH / DOWNSAMPLE_BY ; Height = BUFFER_HEIGHT / DOWNSAMPLE_BY; Format = R16f;};
sampler sVarianceTex {Texture = VarianceTex;};

texture ShadowTex { Width = BUFFER_WIDTH / DOWNSAMPLE_BY ; Height = BUFFER_HEIGHT / DOWNSAMPLE_BY; Format = R16f;};
sampler sShadowTex {Texture = ShadowTex;};

texture BGColorTex { Width = BUFFER_WIDTH / DOWNSAMPLE_BY ; Height = BUFFER_HEIGHT / DOWNSAMPLE_BY; Format = RGBA16f;};
sampler sBGColorTex {Texture = BGColorTex;};

texture LightingTex { Width = BUFFER_WIDTH / DOWNSAMPLE_BY ; Height = BUFFER_HEIGHT / DOWNSAMPLE_BY; Format = RGBA16f;};
sampler sLightingTex {Texture = LightingTex;};

texture BlendedTex { Width = BUFFER_WIDTH / DOWNSAMPLE_BY ; Height = BUFFER_HEIGHT / DOWNSAMPLE_BY; Format = RGBA16f;};
sampler sBlendedTex {Texture = BlendedTex;};

texture LitHistTex { Width = BUFFER_WIDTH / DOWNSAMPLE_BY ; Height = BUFFER_HEIGHT / DOWNSAMPLE_BY; Format = RGBA16f;};
...
texture texMotionVectors { Width = BUFFER_WIDTH / DOWNSAMPLE_BY ; Height = BUFFER_HEIGHT / DOWNSAMPLE_BY; Format = RG16F; };
...
uniform float finder_pow_scale
<
    ui_category = "General";
...
{
	texcoord *= BUFFER_SCREEN_SIZE / DOWNSAMPLE_BY; //convert to pixel index
...
void Lighting(i i, out float3 FinalColor : SV_Target0, out float4 Fog : SV_Target1, out float ShadowOnly : SV_Target2)
{
	...
	for(uint x = 0; x < SOURCE_REGIONS_X; x++) {
        for(uint y = 0; y < SOURCE_REGIONS_Y; y++) {
            ...
            // float light_depth = LDepth(light.xy);
			float light_depth = (LDepth(light.xy));
			float3 light_pos = 1. - UVtoPos(light.xy);
			light.y = 1. - light.y;
			if(light.z) {
				GetLighting(
					FinalColor, spr, Specular, fog, ShadowOnly,
					alpha, position, normal, eyedir, NdotV, F0, i.texcoord, sprite,
					float3(light.xy, light_depth * finder_clamp), UI_LAMP1_PRECISE,
					normalize(tex2D(ReShade::BackBuffer, i.texcoord).rgb),
